...
# loop-engineering thread at session teardown and prints a `/carbonight`
...
if [[ "$HAS_UNCOMMITTED" == "true" || "$HAS_OPEN_LOOP" == "true" ]]; then
  REMINDER_SHOWN="true"
  echo "Unfinished work detected at session end (uncommitted:$HAS_UNCOMMITTED, open_loop:$HAS_OPEN_LOOP) — run /carbonight before ending the session."
fi
