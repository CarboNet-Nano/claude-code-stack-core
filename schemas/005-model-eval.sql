...
SELECT
  ...
  -- Variance (E[x^2] - E[x]^2). USE THIS, NOT `spread`, TO COMPARE STABILITY ACROSS
  -- CELLS. `spread` is max-min, which grows with n purely because more draws have more
  -- chance to hit an extreme — comparing spread between an n=3 and an n=9 cell measures
  -- sample size, not stability. (Learned the hard way on 2026-08-07: sonnet/high n=3
  -- spread 1.67 vs sonnet/xhigh n=9 spread 4.67 looked like a stability difference and
  -- was mostly an n difference.)
  -- Variance, not stddev: stock macOS sqlite3 ships without SQRT (math functions are a
  -- compile-time option), and any SQRT in the view breaks EVERY query on it there.
  -- Take sqrt(variance) at read time if you need stddev; variance orders cells the same.
  ROUND(
