...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'stack_writer') THEN
    ...
    GRANT SELECT, INSERT, UPDATE ON stack.subagent_runs TO stack_writer;
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies
      WHERE schemaname = 'stack' AND tablename = 'subagent_runs' AND policyname = 'subagent_runs_writer_all'
    ) THEN
      CREATE POLICY subagent_runs_writer_all ON stack.subagent_runs FOR ALL TO stack_writer USING (true) WITH CHECK (true);
    END IF;
  END IF;

  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'stack_reader') THEN
    GRANT USAGE ON SCHEMA stack TO stack_reader;
    GRANT SELECT ON stack.subagent_runs TO stack_reader;
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies
      WHERE schemaname = 'stack' AND tablename = 'subagent_runs' AND policyname = 'subagent_runs_reader_select'
    ) THEN
      CREATE POLICY subagent_runs_reader_select ON stack.subagent_runs FOR SELECT TO stack_reader USING (true);
    END IF;
  END IF;
END $$;
