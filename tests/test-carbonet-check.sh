...
# The D18 Broker row reads host state (/etc/stack-broker/config.json). These
# fixtures test the OTHER rows under controlled conditions, so pin the broker
# row to absent — a dev box with a real broker install must not change these
# verdicts. tests/test-org-check-broker-row.sh owns the Broker row.
export STACK_BROKER_SYSTEM_CONFIG=/nonexistent/stack-broker-config.json
...
mkworld() {
  ...
  # ADR-086 D12: the Stack row is receipt-driven. Default fixture world is a
  # healthy, migrated machine (fresh, status:current receipt) so every OTHER
  # check in this suite (Keys/Access/Repo) isn't spuriously dragged into
  # NOT_READY by an absent receipt. Tests that specifically exercise the
  # Stack row overwrite this with write_receipt().
  mkdir -p "$w/home/.claude/state/stack-update"
  write_receipt "$w" '{"status":"current"}'
...
# write_receipt <world> <jq-merge-patch>
# Writes ~/.claude/state/stack-update/receipt.json for the given world,
# merging <jq-merge-patch> (a JSON object) onto a fresh, minimal-but-valid
# stack-update/v1 base (ADR-086 D5) -- as_of defaults to "now" so callers
# only need to specify what differs (status, reason, needs_human, ...).
write_receipt() {
  local w="$1" patch="$2"
  mkdir -p "$w/home/.claude/state/stack-update"
  jq -n --argjson patch "$patch" --arg now "$(date -u +%Y-%m-%dT%H:%M:%SZ)" '
    {
      schema: "stack-update/v1", hook_version: 3, as_of: $now,
      status: null, reason: null, needs_human: false,
      repo: "/nowhere", tier: 4,
      pack_pending: false, purges_pending: 0
    } * $patch' > "$w/home/.claude/state/stack-update/receipt.json"
}
...
run_check() {
  ...
  (
    ...
    export HOME="$w/home"
    # org-check honors the active config dir (profile sessions); a live
    # CLAUDE_CONFIG_DIR leaking in would point it at the REAL receipt.
...
[[ "$n_ok" == "4" ]] && pass "C01(adapted): 4 ✅ rows (Keys/Stack/Repo/Review) on the best-case fixture" \
  || fail "C01(adapted): expected 4 ✅ rows, got $n_ok"
...
# --- C12: an unnamed skipped reason reads as can't-tell, never as failure --
# (ADR-086 D12: repo-missing is a skipped reason not named in the table's
# closed rows -- it must fall to the generic "can't tell" WARN, not omit and
# not fail.)
W="$(mkworld c12)"
write_fake_claude "$W" true
write_fake_curl "$W"
write_receipt "$W" '{"status":"skipped","reason":"repo-missing"}'
out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
grep -q "Can't tell if the stack is current on this machine" <<<"$out" \
  && pass "C12: an unnamed skipped reason (repo-missing) is printed (not suppressed, unlike /goodmorning)" \
  || fail "C12: repo-not-found line missing"
grep -q "⚠️  Stack" <<<"$out" && pass "C12: an unnamed skipped reason is warn, never fail" \
  || fail "C12: Stack row: $(grep Stack <<<"$out")"

# --- C13/T40: needs_human:true (ADR-086 D12) is Stack ❌, names a terminal
# remedy, and drives the verdict to NOT_READY.
W="$(mkworld c13)"
write_fake_claude "$W" true
write_fake_curl "$W"
write_receipt "$W" '{"status":"blocked","reason":"dirty","needs_human":true}'
out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
rc=$?
grep -q "❌  Stack" <<<"$out" && pass "C13/T40: needs_human:true => Stack ❌" || fail "C13/T40: Stack row: $(grep Stack <<<"$out")"
grep -q "terminal" <<<"$out" && pass "C13/T40: needs_human ❌ row names a terminal remedy" \
  || fail "C13/T40: no 'terminal' in the fix line: $(grep -A1 '❌ Stack' <<<"$out")"
[[ "$rc" == "10" ]] && pass "C13/T40: exit 10" || fail "C13/T40: exit was $rc"

# --- C13a2/T40: a missing receipt (self-updater never ran) is a couldn't-look
# ❌, never silence and never a false ✅ (ADR-085 doctrine, applied via D12).
W="$(mkworld c13a2)"
write_fake_claude "$W" true
write_fake_curl "$W"
rm -f "$W/home/.claude/state/stack-update/receipt.json"
out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
rc=$?
grep -q "❌  Stack" <<<"$out" && pass "C13a2/T40: a missing receipt reports Stack ❌ (couldn't-look), never omitted" \
  || fail "C13a2/T40: Stack row: $(grep Stack <<<"$out")"
grep -q "self-updater didn't run" <<<"$out" \
  && pass "C13a2/T40: missing-receipt row names the self-updater, not a version distance" \
  || fail "C13a2/T40: missing-receipt row text wrong: $(grep -A1 '❌ Stack' <<<"$out")"

# --- C13b/T40: status:staged is Stack ⚠️ and names /stack-update.
W="$(mkworld c13b)"
write_fake_claude "$W" true
write_fake_curl "$W"
write_receipt "$W" '{"status":"staged","staged_sha":"0123456789abcdef0123456789abcdef01234567","staged_count":3}'
out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
rc=$?
grep -q "⚠️  Stack" <<<"$out" && pass "C13b/T40: status:staged reports Stack ⚠️" \
  || fail "C13b/T40: Stack row: $(grep Stack <<<"$out")"
grep -q "/stack-update" <<<"$out" && pass "C13b/T40: staged row's fix line names /stack-update" \
  || fail "C13b/T40: fix line missing /stack-update: $(grep -A1 '⚠️  Stack' <<<"$out")"
[[ "$rc" == "20" ]] && pass "C13b/T40: exit 20 (ALMOST READY)" || fail "C13b/T40: exit was $rc"

# --- T40: pack_pending is Stack ⚠️, and the retired "updates itself next
# time" sentence appears nowhere in the script.
W="$(mkworld t40-pack)"
write_fake_claude "$W" true
write_fake_curl "$W"
write_receipt "$W" '{"status":"updated","pack_pending":true}'
out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
grep -q "⚠️  Stack" <<<"$out" && pass "T40: pack_pending:true reports Stack ⚠️" \
  || fail "T40: Stack row: $(grep Stack <<<"$out")"
grep -qF "updates itself next time" "$ORG_CHECK" \
  && fail "T40: the retired 'updates itself next time' sentence is still in scripts/org-check.sh" \
  || pass "T40: 'updates itself next time' appears nowhere in scripts/org-check.sh"

# --- T40: a fresh, healthy receipt reports Stack ✅.
W="$(mkworld t40-healthy)"
write_fake_claude "$W" true
write_fake_curl "$W"
write_receipt "$W" '{"status":"current"}'
out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
grep -q "✅  Stack" <<<"$out" && pass "T40: a fresh, healthy receipt (status:current) reports Stack ✅" \
  || fail "T40: Stack row: $(grep Stack <<<"$out")"
...
set_curl "$W" gemini_models 200
set_curl "$W" access 200
write_receipt "$W" '{"status":"skipped","reason":"unstamped-profile","profile_dir":"~/.claude-team"}'
sp_json="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- --json)"
...
sp_text="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
...
write_receipt "$W" '{"status":"skipped","reason":"unstamped-profile","profile_dir":"~/.claude-team"}'
sp3_text="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
...
set_curl "$W" access 200
write_receipt "$W" '{"status":"skipped","reason":"unstamped-profile","profile_dir":"~/.claude-team"}'
sp4_out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
sp4_rc=$?
...
# --- RG1-RG4: ADR-087 D5 review-gate health row ----------------------------
W="$(mkworld rg1)"
write_fake_claude "$W" true
write_fake_curl "$W"
set_curl "$W" openai_models 200
set_curl "$W" gemini_models 200
set_curl "$W" access 200
rg1_out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
grep -q "Review gate active (warn)" <<<"$rg1_out" \
  && pass "RG1: healthy world shows 'Review gate active (warn)'" \
  || fail "RG1: no healthy review-gate row"
...
set_curl "$W" gemini_models 200
set_curl "$W" access 200
mkdir -p "$W/home/.claude/logs"
NOW_TS="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
printf '{"event":"review_gate","mount":"G1","decision":"deny","class":"high","reason":"machinery","subagent":"implementer","ts":"%s"}\n' \
  "$NOW_TS" > "$W/home/.claude/logs/review-gate.jsonl"
rg2_out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
grep -q "The review gate is broken on this machine" <<<"$rg2_out" \
  && pass "RG2: machinery deny in last 24h -> broken row" \
  || fail "RG2: no broken row despite fresh machinery deny"
grep -q "review-gate-recovery.md" <<<"$rg2_out" \
  && pass "RG2b: FIX line names the recovery runbook" \
  || fail "RG2b: FIX line missing the runbook pointer"
...
set_curl "$W" gemini_models 200
set_curl "$W" access 200
mkdir -p "$W/home/.claude/state/attest/override"
: > "$W/home/.claude/state/attest/override/review-gate.disabled"
rg3_out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
grep -q "switched off but nobody wrote down why" <<<"$rg3_out" \
  && pass "RG3: empty disable file -> no-reason warn row" \
  || fail "RG3: empty disable file did not warn"
...
set_curl "$W" access 200
mkdir -p "$W/home/.claude/state/attest/override"
printf 'RG4-SENTINEL-REASON maintenance window\n' \
  > "$W/home/.claude/state/attest/override/review-gate.disabled"
rg4_out="$(run_check "$W" OPENAI_API_KEY=sk-test GEMINI_API_KEY=sk-test -- )"
grep -q "Review gate active (off)" <<<"$rg4_out" \
  && pass "RG4: reasoned disable file shows mode off" \
  || fail "RG4: reasoned disable file did not show off"
if grep -q "RG4-SENTINEL-REASON" <<<"$rg4_out"; then
  fail "RG4b: attacker-writable disable reason echoed into the report"
else
  pass "RG4b: disable-file reason never echoed"
fi
