...
# ═══ H1-H4 (docs/ADRs/083-declared-skill-aliases.md, D9/D10): the generator
# reads config/aliases.json (declared) instead of scanning skills/*/SKILL.md
# for alias_of. Implemented — scripts/gen-capability-registry.sh reads
# config/aliases.json; H1-H4 exercise the real path and pass.
write_stack_aliases() { # write_stack_aliases <dir> <json>
  mkdir -p "$1/config"
  printf '%s' "$2" > "$1/config/aliases.json"
}

# ═══ H1: regenerated .aliases byte-identical to today's five-entry map ═════
DH1="$TMP/h1"; mkfixture "$DH1"
for id in goodmorning carbonight handbook; do add_skill "$DH1" "$id"; done
write_stack_aliases "$DH1" '{
  "version": 1,
  "aliases": {
    "hi":      { "target": "goodmorning" },
    "hello":   { "target": "goodmorning" },
    "bye":     { "target": "carbonight" },
    "goodbye": { "target": "carbonight" },
    "docs":    { "target": "handbook" }
  }
}'
run_gen "$DH1"
assert_rc "H1: generator exit 0" 0 "$RC"
REGH1="$DH1/config/capability-registry.json"
EXPECT_H1='{"bye":"carbonight","docs":"handbook","goodbye":"carbonight","hello":"goodmorning","hi":"goodmorning"}'
assert_eq "H1: regenerated .aliases byte-identical to today's five-entry map (the migration's proof)" \
  "$EXPECT_H1" "$(jq -Sc '.aliases' "$REGH1" 2>/dev/null)"

# ═══ H2: promoted carbonet synthesized into capabilities[] verbatim ════════
DH2="$TMP/h2"; mkfixture "$DH2"
add_skill "$DH2" goodmorning
CARBONET_DESC="Check that everything is ready before you start working — that you're signed in, that your access is granted, that the stack is up to date, and that this folder is set up. Prints a plain-English ✅/❌ list with a fix for anything that's wrong, plus what changed since you were last here and a suggested first move, and never shows a key or password. Use at the start of a session, when something stops working, or when you're not sure whether your setup is right."
jq -n --arg desc "$CARBONET_DESC" '{
  "version": 1,
  "aliases": {
    "carbonet": { "target": "goodmorning", "mode": "plain", "help": "row", "tools": "Bash, Read", "description": $desc }
  }
}' > "$DH2/config/aliases.json"
run_gen "$DH2"
assert_rc "H2: generator exit 0" 0 "$RC"
REGH2="$DH2/config/capability-registry.json"
EXPECT_H2='{"id":"carbonet","kind":"skill","summary":"Check that everything is ready before you start working — that you'"'"'re signed in, that your access is granted, that the stack is up to date, and that this folder is set up.","invocation":{"slash":"/carbonet","natural_language":"check that everything is ready before you start working — that you'"'"'re signed in, that your access is granted, that the stack is up to date, and that this folder is set up"},"tier_min":0,"user_invocable":true,"model_invocable":false,"recommendable":true}'
GOT_H2="$(jq -Sc '.capabilities[] | select(.id=="carbonet")' "$REGH2" 2>/dev/null)"
assert_eq "H2: synthesized carbonet capabilities[] entry byte-identical to today's committed entry" \
  "$(echo "$EXPECT_H2" | jq -Sc .)" "$GOT_H2"

# ═══ H3: no suffix alias appears in capabilities[] (a promoted one legitimately does) ═
DH3="$TMP/h3"; mkfixture "$DH3"
for id in goodmorning carbonight handbook; do add_skill "$DH3" "$id"; done
jq -n '{
  "version": 1,
  "aliases": {
    "hi":       { "target": "goodmorning" },
    ...
    "goodbye":  { "target": "carbonight" },
    "docs":     { "target": "handbook" },
    "carbonet": { "target": "goodmorning", "mode": "plain", "help": "row", "tools": "Bash, Read", "description": "Employee-facing summary." }
  }
}' > "$DH3/config/aliases.json"
run_gen "$DH3"
assert_rc "H3: generator exit 0" 0 "$RC"
REGH3="$DH3/config/capability-registry.json"
IDS_H3="$(jq -c '[.capabilities[].id] | sort' "$REGH3" 2>/dev/null)"
for suffix in hi hello bye goodbye docs; do
  assert_not_contains "H3: suffix alias '$suffix' does not appear in capabilities[]" "$IDS_H3" "\"$suffix\""
done
assert_contains "H3: promoted alias 'carbonet' legitimately appears in capabilities[]" "$IDS_H3" "\"carbonet\""

# ═══ H4: T18's count invariant still holds now that stubs are not scanned ═══
DH4="$TMP/h4"; mkfixture "$DH4"
add_skill "$DH4" skill-one
add_skill "$DH4" skill-two
add_agent "$DH4" agent-one
for i in 1 2; do
  id="h4-hook-$i"
  write_hook "$DH4" "$id" "# summary: H4 count fixture hook $i."
  wire_no_matcher "$DH4/hooks/hooks.json" UserPromptSubmit "hooks/$id.sh"
  add_to_manifest "$DH4" 0 "hooks/$id.sh"
done
jq -n '{
  "version": 1,
  "aliases": {
    "hi":       { "target": "skill-one" },
    "hello":    { "target": "skill-one" },
    "bye":      { "target": "skill-two" },
    "goodbye":  { "target": "skill-two" },
    "docs":     { "target": "skill-one" },
    "carbonet": { "target": "skill-one", "mode": "plain", "help": "row", "tools": "Bash, Read", "description": "Employee-facing summary." }
  }
}' > "$DH4/config/aliases.json"
run_gen "$DH4"
assert_rc "H4: generator exit 0" 0 "$RC"
REGH4="$DH4/config/capability-registry.json"
EXPECT_HOOKS_H4=$(hook_glob_count "$DH4")
EXPECT_SKILLS_H4=$(skill_glob_count "$DH4")
EXPECT_AGENTS_H4=$(agent_glob_count "$DH4")
EXPECT_TOTAL_H4=$((EXPECT_HOOKS_H4 + EXPECT_SKILLS_H4 + EXPECT_AGENTS_H4 + 1))  # +1 for the one promoted alias
assert_eq "H4: total entry count == skills+agents+hooks glob counts + promoted aliases (5 suffix words contribute zero, not scanned as stubs)" \
  "$EXPECT_TOTAL_H4" "$(jq '.capabilities|length' "$REGH4" 2>/dev/null)"
