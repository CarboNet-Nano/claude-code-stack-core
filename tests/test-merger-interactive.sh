...
# A sandbox (e.g. the ADR-063 Bash fence) can deny pty allocation entirely;
# expect then dies with "The system has no more ptys". Same honesty contract
# as the expect-absent skip above: skip, never fake a pass or fail.
if ! expect -c 'if {[catch {spawn true} err]} {exit 1}; expect eof' > /dev/null 2>&1; then
  echo "SKIP: no pty available (sandboxed?) — interactive merge path not exercised"
  exit 0
fi
