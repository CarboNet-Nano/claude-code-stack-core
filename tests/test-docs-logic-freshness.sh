...
if [[ -f "$COMPILE" ]] && command -v jq >/dev/null 2>&1; then
  G_HOME="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; mkdir -p "$G_HOME/.claude/session-state" "$G_HOME/.claude/config" "$G_HOME/.claude/logs"
  G_REPO="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; mkdir -p "$G_REPO/.claude"
  ...
  assert_not_contains "G3b: compiler emits no dead Write(./docs/user/.meta/**) -- the harness honors Edit rules only" "$DENY_1" "Write(./docs/user/.meta/**)"
...
# Ceiling bumped 24 -> 25 for the P1b PM roster seat (agents/pm.md,
# ADR-060/REQ-160) — a deliberate, separately-approved addition unrelated to
# ADR-050's docs-logic pipeline. R1's actual guard (ADR-050 D1/D2 took no
# seat) still lives in R2/R3 below.
assert_eq "R1: agents/ still holds 25 agent files" "25" "$(ls "$REPO_ROOT"/agents/*.md | wc -l | tr -d ' ')"
# R2 was a byte-frozen pin of ADR-003 to a base commit; that broke on the
# first legitimate roster amendment (issue #151, the P1b pm seat). What
# ADR-050 D1/D2 actually promise is narrower: the docs-logic pipeline never
# claims a roster seat. Assert that content claim directly — ADR-003 must
# never name a docs-logic role, however often it is otherwise amended.
if grep -qiE 'logic-extractor|docs-logic seat|logic extractor' "$REPO_ROOT/docs/ADRs/003-21-subagents.md"; then
  fail "R2: ADR-003 names a docs-logic roster role — ADR-050 D1/D2 says no seat is taken"
else
  pass "R2: ADR-003 names no docs-logic roster role (no seat taken)"
fi
