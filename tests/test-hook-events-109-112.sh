...
echo "$OUT" | grep -q "/carbonight" && pass "109.1: reminder printed when uncommitted work exists" \
...
echo "$OUT" | grep -q "/carbonight" && pass "109.5: reminder printed for an open loop thread (clean git tree)" \
