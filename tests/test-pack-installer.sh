...
# 27 (G1, ADR-083 D8): validate_pack gains an alias check — every listed
# malformed/dangerous shape of config/aliases.org.json is rejected in Phase 0,
# before any write. Implemented — validate_pack reads config/aliases.org.json
# and every sub-case below is rejected as expected.
ALIASCORE="$TMP/alias-core"
cp -R "$CORE" "$ALIASCORE"
mkdir -p "$ALIASCORE/skills/handbook"
cat > "$ALIASCORE/skills/handbook/SKILL.md" << 'EOF'
# Handbook
Core fixture target skill.
EOF
mkdir -p "$ALIASCORE/config"
cat > "$ALIASCORE/config/aliases.json" << 'EOF'
{
  "version": 1,
  "aliases": {
    "docs": { "target": "handbook" },
    "carbonet": { "target": "sample-skill", "help": "row", "description": "Employee-facing summary." }
  }
}
EOF

mk_alias_pack() { # mk_alias_pack <name> <aliases.org.json content>
  local dir="$TMP/aliaspack-$1"
  cp -R "$PACK" "$dir"
  mkdir -p "$dir/config"
  printf '%s' "$2" > "$dir/config/aliases.org.json"
  echo "$dir"
}

assert_alias_pack_rejected() { # assert_alias_pack_rejected <label> <name> <content>
  local label="$1" dir
  dir="$(mk_alias_pack "$2" "$3")"
  if validate_pack "$dir" "$ALIASCORE" > /dev/null 2>&1; then
    echo "FAIL: G1 $label — pack was accepted, should have been rejected"
    exit 1
  fi
}

assert_alias_pack_rejected "malformed JSON" "malformed" '{ not valid json'
assert_alias_pack_rejected "wrong version" "wrongversion" \
  '{"version":2,"aliases":{"standup":{"target":"sample-skill"}}}'
assert_alias_pack_rejected "word shadows an existing capability id" "shadow" \
  '{"version":1,"aliases":{"sample-skill":{"target":"handbook"}}}'
assert_alias_pack_rejected "word takes a stack promoted alias" "promotedtakeover" \
  '{"version":1,"aliases":{"carbonet":{"target":"handbook"}}}'
assert_alias_pack_rejected "target is not a kind:skill capability" "badtarget" \
  '{"version":1,"aliases":{"standup":{"target":"no-such-skill"}}}'
assert_alias_pack_rejected "target is itself an alias (chain)" "chain" \
  '{"version":1,"aliases":{"mydocs":{"target":"docs"}}}'
assert_alias_pack_rejected "help: row outside the stack source" "helprow" \
  '{"version":1,"aliases":{"standup":{"target":"sample-skill","help":"row","description":"x"}}}'
assert_alias_pack_rejected "unknown field" "unknownfield" \
  '{"version":1,"aliases":{"standup":{"target":"sample-skill","bogus":true}}}'

echo "PASS: G1 validate_pack rejects every ADR-083 D8 aliases.org.json shape"

# 28 (G2): the review gate prints stack/org alias word changes in words —
# "changes /docs (was → /handbook, now → /acme-handbook)" shape (D7).
mkdir -p "$ALIASCORE/skills/acme-handbook"
cat > "$ALIASCORE/skills/acme-handbook/SKILL.md" << 'EOF'
# Acme handbook
Core fixture repoint target.
EOF
REPOINTPACK="$(mk_alias_pack "repoint" '{"version":1,"aliases":{"docs":{"target":"acme-handbook"}}}')"
CLAUDE_REPOINT="$TMP/home-repoint/.claude"
mkdir -p "$CLAUDE_REPOINT"
G2_PREVIEW="$(install_pack "$REPOINTPACK" "$CLAUDE_REPOINT" "$ALIASCORE" 2>&1)" && {
  echo "FAIL: G2 preview install without 'reviewed' should not have applied"
  exit 1
}
for needle in "/docs" "/handbook" "/acme-handbook" "was" "now"; do
  if [[ "$G2_PREVIEW" != *"$needle"* ]]; then
    echo "FAIL: G2 review-gate preview missing '$needle' — word changes must be printed in words, not a JSON path"
    echo "  preview was: $G2_PREVIEW"
    exit 1
  fi
done

echo "PASS: G2 review gate prints the alias word change in words"

# 29 (G3): config/aliases.org.json lands verbatim (plain copy, no deep-merge
# into the stack's own config/aliases.json — D5). ~/.claude/config/aliases.json
# (the stack's own file, landed at core-install time, not by the pack) is
# untouched by this pack.
ORGLANDPACK="$(mk_alias_pack "orgland" '{"version":1,"aliases":{"standup":{"target":"sample-skill"}}}')"
CLAUDE_ORGLAND="$TMP/home-orgland/.claude"
mkdir -p "$CLAUDE_ORGLAND"
cp "$ALIASCORE/config/aliases.json" "$CLAUDE_ORGLAND/config/aliases.json" 2>/dev/null || {
  mkdir -p "$CLAUDE_ORGLAND/config"
  cp "$ALIASCORE/config/aliases.json" "$CLAUDE_ORGLAND/config/aliases.json"
}
STACK_ALIASES_BEFORE="$(cat "$CLAUDE_ORGLAND/config/aliases.json")"
install_pack "$ORGLANDPACK" "$CLAUDE_ORGLAND" "$ALIASCORE" "reviewed" > /dev/null
if ! diff -q "$CLAUDE_ORGLAND/config/aliases.org.json" "$ORGLANDPACK/config/aliases.org.json" > /dev/null 2>&1; then
  echo "FAIL: G3 config/aliases.org.json did not land verbatim"
  exit 1
fi
STACK_ALIASES_AFTER="$(cat "$CLAUDE_ORGLAND/config/aliases.json")"
if [[ "$STACK_ALIASES_BEFORE" != "$STACK_ALIASES_AFTER" ]]; then
  echo "FAIL: G3 ~/.claude/config/aliases.json (stack source) was touched by the pack — must be untouched, no deep-merge"
  exit 1
fi
if [[ -e "$CLAUDE_ORGLAND/config/aliases.json.pack-overrides" ]]; then
  echo "FAIL: G3 aliases.org.json landing produced a .pack-overrides row — it must be a plain add, not a merge"
  exit 1
fi

echo "PASS: G3 aliases.org.json lands verbatim; the stack's own aliases.json is untouched"
