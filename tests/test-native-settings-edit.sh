...
# --- ADR-063 phase 3: sandbox.network.strictAllowlist leaf ----------------
# Docs scope-restrict this key to user/managed/CLI settings — project scope
# must be refused outright; user scope needs the usual --confirm-global.
setup
assert_rc 2 "strictAllowlist refused at project scope" -- --path /sandbox/network/strictAllowlist --value true
HOMEDIR="$WORK/home"; mkdir -p "$HOMEDIR/.claude"
echo '{"model":"sonnet"}' > "$HOMEDIR/.claude/settings.json"
out="$(CLAUDE_SETTINGS_HOME="$HOMEDIR" python3 "$PY" --path /sandbox/network/strictAllowlist --value true --scope user 2>&1)"; rc=$?
[[ "$rc" == 2 ]] && PASS=$((PASS+1)) || { FAIL=$((FAIL+1)); echo "FAIL: strictAllowlist user scope without --confirm-global (rc=$rc)"; }
out="$(CLAUDE_SETTINGS_HOME="$HOMEDIR" python3 "$PY" --path /sandbox/network/strictAllowlist --value true --scope user --confirm-global 2>&1)"; rc=$?
[[ "$rc" == 0 ]] && PASS=$((PASS+1)) || { FAIL=$((FAIL+1)); echo "FAIL: strictAllowlist user scope with --confirm-global should write (rc=$rc); out: $out"; }
got="$(python3 -c 'import json,sys;d=json.load(open(sys.argv[1]));print(json.dumps(d.get("sandbox",{}).get("network",{}).get("strictAllowlist")))' "$HOMEDIR/.claude/settings.json")"
[[ "$got" == "true" ]] && PASS=$((PASS+1)) || { FAIL=$((FAIL+1)); echo "FAIL: strictAllowlist not written as boolean true (got $got)"; }
mdl="$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1])).get("model"))' "$HOMEDIR/.claude/settings.json")"
[[ "$mdl" == "sonnet" ]] && PASS=$((PASS+1)) || { FAIL=$((FAIL+1)); echo "FAIL: siblings clobbered by strictAllowlist write (model=$mdl)"; }
out="$(CLAUDE_SETTINGS_HOME="$HOMEDIR" python3 "$PY" --path /sandbox/network/strictAllowlist --value yes --scope user --confirm-global 2>&1)"; rc=$?
[[ "$rc" == 2 ]] && PASS=$((PASS+1)) || { FAIL=$((FAIL+1)); echo "FAIL: strictAllowlist non-literal bool must be refused (rc=$rc)"; }
echo '{"sandbox":"oops"}' > "$HOMEDIR/.claude/settings.json"
out="$(CLAUDE_SETTINGS_HOME="$HOMEDIR" python3 "$PY" --path /sandbox/network/strictAllowlist --value true --scope user --confirm-global 2>&1)"; rc=$?
[[ "$rc" == 2 ]] && PASS=$((PASS+1)) || { FAIL=$((FAIL+1)); echo "FAIL: non-object sandbox node must refuse, not clobber (rc=$rc)"; }
teardown
