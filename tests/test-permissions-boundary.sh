...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T1="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T1")
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T2="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T2")
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T4="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T4")
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T5="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T5")
...
REPO_T6="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T6"); mkdir -p "$REPO_T6/.claude"
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T8="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T8")
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T9="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T9")
write_snapshot "$CUR_HOME" "stripe"
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T11="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T11")
...
CUR_HOME="$(new_home)"
export HOME="$CUR_HOME"
REPO_T12="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T12")
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T13="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T13")
write_snapshot "$CUR_HOME" "stripe"
...
CUR_HOME2="$(new_home)"
REPO_T13B="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T13B")
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T14="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T14")
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T15="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T15")
write_snapshot "$CUR_HOME" "supabase"
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T16="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T16")
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T17="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T17")
...
assert_contains "T18: written floor includes the new baseline self-protection rule" "$USER_DENY" "Edit(~/.claude/config/permissions-baseline.json)"
USER_DENY_COUNT_BEFORE="$(jq -r '.permissions.deny | length' "$CUR_HOME/.claude/settings.json")"
# 9, not 16: baseline 1.6.1 narrows file_tools to ["Edit"] and retires the three
# literal Write() rules ADR-071 D12 added, because the harness itself refuses
# them -- it warns "Write(path) is not matched by file permission checks, only
# Edit(path) rules are" (observed directly, 2026-08-12). So floor.deny(4
# literals: 2 Read + Edit of ./.claude/settings.json and .local.json) +
# path_rules(5 paths x 1 tool) = 9 unique rules. Every path D12 set out to
# protect still has an Edit rule; only the dead twins are gone.
assert_eq "T18: floor is 9 rules after the first user-scope compile (baseline 1.6.1)" "9" "$USER_DENY_COUNT_BEFORE"
...
assert_eq "T18: floor survives --prune-user, which strips only the retired Write() twins" "9" "$USER_DENY_COUNT_AFTER"

# T18b — no Write() rule may ever reach a compiled deny list. The harness does
# not match them and warns on each one by name, so any that appear are both dead
# weight and a visible startup warning. This is the regression guard: it fails on
# the whole class, not on the three rules that were retired in 1.6.1.
USER_WRITE_RULES="$(jq -r '[.permissions.deny[]? | select(startswith("Write("))] | length' "$CUR_HOME/.claude/settings.json")"
assert_eq "T18b: compiled user floor contains zero dead Write() rules" "0" "$USER_WRITE_RULES"
...
# ─────────────────────────────────────────────────────────────────────────
CUR_HOME="$(new_home)"
REPO_T19="$(mktemp -d)" || { echo "FATAL: mktemp failed" >&2; exit 1; }; CLEANUP_DIRS+=("$REPO_T19")
