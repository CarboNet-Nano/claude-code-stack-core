...
# --- D9: ADR-087's amendment — both override tiers move under the protected
# namespace, gain a mandatory reason, and the legacy files are honoured once.
# build_repo() reuses one fixed path and wipes it, so D9 gets its own repo
# rather than destroying the shared fixture's minted-token state.
D9_REPO="$TMP/repo-d9"; rm -rf "$D9_REPO"; mkdir -p "$D9_REPO/.claude"
( cd "$D9_REPO" && git init -q -b main && git config user.email t@t.t && git config user.name t
  echo x > README.md && git add -A && git commit -qm init )
D9_REPO="$(cd "$D9_REPO" && git rev-parse --show-toplevel)"
mkdir -p "$D9_REPO/.claude"
echo '{"stack_tier":4,"guards":{"usage_check_gate":"on"}}' > "$D9_REPO/.claude/stack-config.json"
D9_OVR_DIR="$HOME/.claude/state/attest/override"
D9_RH="$(bash -c "source '$REPO_ROOT/scripts/lib/usage-check-common.sh'; uc_repo_hash '$D9_REPO'")"

# An EMPTY machine-wide disable file does not disable (same rule as the
# review gate's — one shape of mistake, one behaviour).
mkdir -p "$D9_OVR_DIR"
: > "$D9_OVR_DIR/usage-check-gate.disabled"
OUT="$(gate_payload "$D9_REPO" "reviewer" "no target" "sess-D9a" | bash "$GATE_HOOK" 2>/dev/null)"
is_deny "$OUT" && pass "D9: empty machine-wide disable file does not disable" \
  || fail "D9: empty disable file silenced the gate: ${OUT:-<silent>}"

# With a reason, it disables.
printf 'maintenance window, ticket 123\n' > "$D9_OVR_DIR/usage-check-gate.disabled"
OUT="$(gate_payload "$D9_REPO" "reviewer" "no target" "sess-D9b" | bash "$GATE_HOOK" 2>/dev/null)"
[[ -z "$OUT" ]] && pass "D9: machine-wide disable with a reason disables" \
  || fail "D9: reasoned disable file did not disable: $OUT"
rm -f "$D9_OVR_DIR/usage-check-gate.disabled"

# The legacy bare file is honoured ONCE and removed — never converted into a
# standing disable nobody chose.
: > "$HOME/.claude/usage-check-gate.disabled"
OUT="$(gate_payload "$D9_REPO" "reviewer" "no target" "sess-D9c" | bash "$GATE_HOOK" 2>/dev/null)"
[[ -z "$OUT" ]] && pass "D9: legacy machine-wide disable is honoured once" \
  || fail "D9: legacy disable was not honoured: $OUT"
[[ ! -f "$HOME/.claude/usage-check-gate.disabled" ]] \
  && pass "D9: legacy machine-wide file is removed after being honoured" \
  || fail "D9: legacy file survived"
if [[ -f "$D9_OVR_DIR/usage-check-gate.disabled" ]]; then
  fail "D9: honouring the legacy file created a standing disable"
else
  pass "D9: honouring the legacy file creates no standing disable"
fi
OUT="$(gate_payload "$D9_REPO" "reviewer" "no target" "sess-D9d" | bash "$GATE_HOOK" 2>/dev/null)"
is_deny "$OUT" && pass "D9: the gate is armed again on the next dispatch" \
  || fail "D9: gate stayed disabled after the one-time honour: ${OUT:-<silent>}"

# Repo-once override now lives outside the repo, keyed by repo hash, and is
# consumed exactly once.
mkdir -p "$D9_OVR_DIR/usage-check-once"
printf 'one-off, ticket 456\n' > "$D9_OVR_DIR/usage-check-once/$D9_RH"
OUT="$(gate_payload "$D9_REPO" "reviewer" "no target" "sess-D9e" | bash "$GATE_HOOK" 2>/dev/null)"
[[ -z "$OUT" ]] && pass "D9: namespaced repo-once override is honoured" \
  || fail "D9: namespaced repo-once override ignored: $OUT"
[[ ! -f "$D9_OVR_DIR/usage-check-once/$D9_RH" ]] \
  && pass "D9: repo-once override is consumed" \
  || fail "D9: repo-once override was not consumed"
OUT="$(gate_payload "$D9_REPO" "reviewer" "no target" "sess-D9f" | bash "$GATE_HOOK" 2>/dev/null)"
is_deny "$OUT" && pass "D9: consumed repo-once override does not apply twice" \
  || fail "D9: override applied a second time: ${OUT:-<silent>}"

# The override file no longer lives in the repo, so it cannot be committed —
# which is why the untracked-only rule is deleted rather than reimplemented.
if [[ -e "$D9_REPO/.claude/.usage-check-override-once" ]]; then
  fail "D9: an override file was written back into the repo"
else
  pass "D9: no override file is written inside the repo"
fi
