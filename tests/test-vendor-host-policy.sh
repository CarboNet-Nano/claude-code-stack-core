...
assert_eq "P1: normal -> all six governed hosts present" "6" "$(echo "$OUT_P1" | jq '.allowed_hosts | length')"
...
assert_eq "P2: sensitive -- with shipped vendor-hosts.json only anthropic+openai are cleared (gemini closed, ADR-071 D15 #6)" \
  "2" "$(echo "$OUT_P2" | jq '.allowed_hosts | length')"
...
assert_eq "A2: normal -> all six governed hosts present" "6" "$(echo "$OUT_A2" | jq '.allowed_hosts | length')"
...
assert_eq "P5: confidential -> api.anthropic.com only" "1" "$(echo "$OUT_P5" | jq '.allowed_hosts | length')"
assert_eq "P5: confidential allowed host is api.anthropic.com" "api.anthropic.com" "$(echo "$OUT_P5" | jq -r '.allowed_hosts[0]')"
...
assert_eq "P9: allowedDomains created at normal" "6" "$(jq '.sandbox.network.allowedDomains | length' "$R9/.claude/settings.json")"
...
assert_eq "F2: unparseable stack-config -> Anthropic-only" "api.anthropic.com" "$(echo "$OUT_F2" | jq -r '.allowed_hosts[0]')"
...
assert_eq "H7: level-changing edit triggers a recompile" "2" "$(jq '.sandbox.network.allowedDomains | length' "$RH7/.claude/settings.json")"
...
assert_eq "SEC3: the tightened host list is actually written despite the forged marker" \
  "2" "$(echo "$OUT_SEC3" | jq '.allowed_hosts | length')"
