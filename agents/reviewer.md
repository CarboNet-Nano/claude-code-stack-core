...
dispatch_when: adversarial cold-read of every non-trivial diff before merge
---
...
2. Run the adversarial review on the **routed** engine/model from Step 0.5, scoped to the diff:
   - **routine / local:** `ollama run "$RR_MODEL"` with the review prompt below (diff piped in). (Local Qwen mints no receipt — routine diffs are below the gate's radar by design.)
   - **high or escalation / OpenAI family:** pipe the diff into
     **`scripts/panel-review.sh`** — the sole sanctioned seat runner (ADR-087
     ...
     mints the review receipt ADR-087's gates check; a direct vendor call
     mints nothing and does not count. Keep it diff-scoped — never a
     whole-repo cold read:
     ```bash
     ...
     EOF
     git diff <base>..<head> | \
       "$PANEL" reviewer --diff <base>..<head> --prompt-file /tmp/reviewer-prompt.txt
     ```
     ...
     the minting hook reads it. Omit `--prompt-file` to use the seat's
     built-in prompt.
   - Review prompt (both engines): the text shown in the prompt file above.
3. Capture the routed engine's output verbatim (Qwen for routine, the OpenAI family via the panel runner for high/escalation).
4. Structure it into the handoff format below. Do not soften, drop, or override Codex's findings.
5. **Transport is handled by the runner's vendor helper, not by you (ADR-030).** The requirement (ADR-011, ADR-015) is review by a **non-Claude model family** — the *model*, not the *binary*. The helper resolves `codex_transport` itself:
