...
dispatch_when: before architect on non-trivial features, to challenge what is being built
---
...
- **Escalate by judgment** to the OpenAI family when the feature is genuinely
  high-stakes (auth/payments/financial/regulated) or Qwen's critique comes back
  shallow — pipe the feature brief into **`scripts/panel-review.sh`** — the
  sole sanctioned seat runner (ADR-087 D3a; routes through the ADR-030 OpenAI
  helper and mints the review receipt ADR-087's gates check — a direct vendor
  call mints nothing and does not count).
...
# high / escalation — same prompt through the panel runner (mints the receipt):
PANEL="${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/scripts/panel-review.sh"
cat > /tmp/product-critic-prompt.txt <<'EOF'
You are an adversarial product critic. Challenge the proposed feature in the piped brief: (1) What problem does this solve, in specific user terms? (2) Who exactly is the user? (3) What's the cost of doing nothing? (4) What metric does this move? (5) Is this the simplest version that delivers the metric? (6) What's the failure mode in production? (7) Is this the right next thing to build vs the opportunity cost? Be direct, no hedging. End with a recommendation: proceed as scoped / proceed narrower / defer / reject.
EOF
cat <feature-brief-file> | \
  "$PANEL" product-critic --subject <feature-brief-file> --prompt-file /tmp/product-critic-prompt.txt
# Stdin is the context channel (D3a). The runner prints the critique, then one
# REVIEW_EVIDENCE:v1 line — leave it alone; the minting hook reads it.
```
