...
# org.json is resolved with the same trusted two-tier lookup as
# vendor-hosts.json above, and for the same reason: REPO_ROOT is the project
# under test, so a repo-local org.json must never be able to widen its own
# allowlist. Only org.access_url is read, and only its hostname (see
# org_access_hosts() below, which refuses anything that is not a plain https
# host). It is org infrastructure, NOT model egress, so it is deliberately
# kept out of the vendor universe: it never enters all_hosts, is never
# subject to policy_for(), and the F8/T7 vendor drift gates stay exact.
ORG_CONFIG="$HOME/.claude/config/org.json"
[[ -f "$ORG_CONFIG" ]] || ORG_CONFIG="${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/config/org.json"
...
(repo_root, cfg_path, vendor_hosts_path, baseline_path, snapshot_path,
 org_config_path, managed_path, project_settings_path, local_settings_path,
 user_settings_path, sidecar_path, receipt_path, cloud_str,
 disabled_str) = sys.argv[1:15]
...
ORG_HOST_RE = re.compile(r"^[a-z0-9]([a-z0-9-]*[a-z0-9])?(\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)+$")


def org_access_hosts(path):
    """Hostname of org.access_url, or empty. Never raises, never guesses.

    Refused, silently and by design: a non-https scheme, credentials, a port,
    an IP literal, a wildcard, anything with a path-traversal shape. The
    allowlist is a security surface, so an unparseable value must contribute
    nothing rather than a best-effort guess.
    """
    data, existed, broken = load_json(path)
    if not existed or broken or not isinstance(data, dict):
        return set()
    org = data.get("org")
    url = org.get("access_url") if isinstance(org, dict) else None
...
# The org access app (scripts/org-check.sh Check 2) is first-party org
# infrastructure carrying no prompt content, so it is allowed at EVERY
# sensitivity level -- including restricted, where the vendor universe
# collapses to the runtime host alone. It is added after denied_hosts is
# computed so it can never appear in the removal set, and it is never added
# to all_hosts, which keeps the vendor drift gates exact.
org_hosts = org_access_hosts(org_config_path)
allowed_hosts = allowed_hosts | org_hosts
denied_hosts = denied_hosts - org_hosts
...
STDOUT_JSON="$(echo "$PLAN_JSON" | python3 "$LIB_PY" \
  ...
  --sidecar "$SIDECAR" \
  --receipt "$RECEIPT" \
  --vendor-hosts "$VENDOR_HOSTS" \
  --org-config "$ORG_CONFIG" 2>"$ERR_TMP")"
rc=$?
if [[ $rc -ne 0 ]]; then
  cat "$ERR_TMP" >&2
  rm -f "$ERR_TMP"
