...
# when the freshness helper reports `unstamped-profile <dir>`.
PROFILE_PRESENT=0
PROFILE_STATUS=""
PROFILE_REASON=""
PROFILE_DIR=""
...
FRESH_LIB="$(resolve_lib lib/stack-freshness.sh)"
if [[ ! -f "$FRESH_LIB" ]]; then
  STACK_STATUS="warn"; STACK_REASON="helper-missing"
  STACK_DETAIL="Can't tell — stack helper not installed"
  STACK_FIX="$(generic_warn_fix)"
else
  STACK_TOKEN="$(bash "$FRESH_LIB" --oneline 2>/dev/null)"
  case "$STACK_TOKEN" in
    current)
      STACK_STATUS="ok"; STACK_DETAIL="Stack up to date" ;;
    "0 behind"|"0 behind — run update.sh")
      # An older installed copy of stack-freshness.sh reports a distance of 0
      # when the install is AHEAD of the remote. There is nothing to apply, and
      # "0 versions behind" is unfixable advice. Newer helpers emit `current`.
      STACK_STATUS="ok"; STACK_DETAIL="Stack up to date" ;;
    *" behind — run update.sh"|*" behind")
      # Both token shapes: the second is current, the first is what an older
      # installed copy of stack-freshness.sh still emits. A machine can be
      # running a stack older than this script.
      N_BEHIND="${STACK_TOKEN%% behind*}"
      STACK_STATUS="fail"; STACK_REASON="behind"
      STACK_DETAIL="Stack is ${N_BEHIND} versions behind"
      # Not "run update.sh" and not "ask an admin": update.sh refuses on a dirty
      # tree, so a by-hand instruction can be unfollowable, and the maintainer
      # reading this may BE the support contact. The update applies itself at
      # the next clean session start; escalation is only for when it doesn't.
      STACK_FIX="Nothing to do — it updates itself next time you start a session. Still behind after that? $SUPPORT_CONTACT."
      ;;
    repo-not-found)
      STACK_STATUS="warn"; STACK_REASON="no-source-repo"
      STACK_DETAIL="Can't tell if the stack is current on this machine"
      STACK_FIX="$(generic_warn_fix)"
      ;;
    "unstamped-profile "*)
      # A non-default profile (~/.claude-<name>) with no install stamp at
      # all -- distinct from Check 3's own "unstamped" (a default ~/.claude
      # install missing its stamp). Rendered as its own stack-profile item
      # below, loudly, rather than folded into the generic Stack row.
      PROFILE_PRESENT=1
      PROFILE_DIR="${STACK_TOKEN#unstamped-profile }"
      ...
      # The generic Stack row still reflects "can't tell" -- true and
      # unchanged behavior for every other unstamped case.
      STACK_STATUS="warn"; STACK_REASON="unstamped"
      STACK_DETAIL="Can't tell — the stack install has no version stamp"
      STACK_FIX="$(generic_warn_fix)"
      ;;
    # Every remaining value -- unstamped, unknown, empty, or a word this
    # script has never seen -- means the same thing: we cannot tell.
    *)
      STACK_STATUS="warn"; STACK_REASON="unstamped"
      STACK_DETAIL="Can't tell — the stack install has no version stamp"
      STACK_FIX="$(generic_warn_fix)"
