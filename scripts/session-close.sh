    ...
    if [[ -z "$wt" || ! -d "$wt" ]]; then
      ...
      if git -C "$root" worktree add -q -b "$wb" "$wt" "origin/$default_branch" >/dev/null 2>&1; then
        ...
        if git -C "$wt" add -- "${paths[@]}" >/dev/null 2>&1 && \
           git -C "$wt" commit -q -m "docs(handoff): ${stamp} session handoff + archive" -- "${paths[@]}" >/dev/null 2>&1 && \
           git -C "$wt" push -q origin "HEAD:$wb" >/dev/null 2>&1; then
          commit_sha="$(git -C "$wt" rev-parse --verify -q HEAD 2>/dev/null)"
          committed=true; landing="pr"; ref="$wb"
          pr_url="$(cd "$wt" && gh pr create --base "$default_branch" --head "$wb" \
                      --title "docs(handoff): ${stamp} session handoff + archive" \
                      --body "Session handoff and archive. Generated by session-close.sh handoff-write." 2>/dev/null | tail -1)"
        else
