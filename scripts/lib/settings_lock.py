...
_SP_ORG_HOST_RE = re.compile(r"^[a-z0-9]([a-z0-9-]*[a-z0-9])?(\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)+$")


def _sp_org_access_hosts(org_config_path):
    """Independent, fresh-from-disk derivation of the org access host --
    deliberately duplicated from scripts/sandbox-policy-compile.sh's
    org_access_hosts() rather than taken from the plan, for exactly the
    reason _sp_load_vendor_records() is: the writer must verify the plan
    against what disk says now, never against the compiler's claim about
    its own plan.

    This host is org infrastructure (the sign-in app scripts/org-check.sh
    pings), not model egress. It is therefore allowed at EVERY sensitivity
    level and is kept out of the vendor records entirely, so the F8 drift
    gate over HARDCODED_VENDOR_HOSTS stays exact. Anything that is not a
    plain https hostname contributes nothing.
    """
    if not org_config_path:
        return set()
    try:
        with open(org_config_path, "r", encoding="utf-8") as fh:
            data = json.load(fh)
    except (OSError, ValueError):
        return set()
    if not isinstance(data, dict):
    ...
    if not isinstance(url, str) or not url.startswith("https://"):
        return set()
    host = url[len("https://"):].split("/", 1)[0].strip().lower()
    if "@" in host or ":" in host or "*" in host:
        return set()
    if not _SP_ORG_HOST_RE.match(host):
        return set()
...
def apply_sandbox_policy(target, local_target, sidecar_path, receipt_path,
                         vendor_hosts_path, org_config_path=None):
    plan = _read_plan_stdin()
    if not isinstance(plan, dict):
        raise IOErrorSanitized("plan on stdin is not a JSON object")

    vendor_records = _sp_load_vendor_records(vendor_hosts_path)
    org_hosts = _sp_org_access_hosts(org_config_path)
    universe = {r["host"] for r in vendor_records} | org_hosts
    ...
    def mutate_project(data):
        ...
        current_allowed = _sp_policy_for_level(current_level, vendor_records) | org_hosts
...
def build_parser():
    ...
    p.add_argument("--org-config", default=None)
    return p
...
def main(argv=None):
    ...
    try:
        if args.apply_sandbox_policy:
            ...
            org_config_path = args.org_config or os.path.join(
                _claude_home(), ".claude", "config", "org.json")
