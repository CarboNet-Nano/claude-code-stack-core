...
#
# ADR-087 D3b: set OAIR_EVIDENCE_OUT to a file path (caller-created via
# mktemp, truncated before the call) to receive one JSON evidence object on
# the SUCCESS (API, http 200) path only. NOT an exported global -- per-
# invocation sidecar, read once by the caller. stdout contract is UNCHANGED.
# The CLI transport never writes a sidecar (no HTTP status / usage to report
# for that path) -- scripts/panel-review.sh forces api transport for D3f's
# non-vacuity floors to be checkable at all.
#
...
EOF
  ...
  # ADR-087 D3b: success-path-only sidecar. See gemini-api.sh's identical block.
  if [[ -n "${OAIR_EVIDENCE_OUT:-}" ]]; then
    jq -nc \
      --arg vendor "openai" --arg family "openai" \
      --arg model_requested "$model" \
      --arg model_returned "$(printf '%s' "$resp" | jq -r '.model // empty' 2>/dev/null)" \
      --argjson http_status "$http" \
      --arg response_id "$(printf '%s' "$resp" | jq -r '.id // empty' 2>/dev/null)" \
      --argjson input_tokens "$(printf '%s' "$resp" | jq -r '.usage.prompt_tokens // 0' 2>/dev/null)" \
      --argjson output_tokens "$(printf '%s' "$resp" | jq -r '.usage.completion_tokens // 0' 2>/dev/null)" \
      '{vendor:$vendor, family:$family, model_requested:$model_requested,
      ...
      > "$OAIR_EVIDENCE_OUT" 2>/dev/null || true
  fi
