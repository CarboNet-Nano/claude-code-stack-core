...
_cfp_base_url_ok() {
  [[ "${CFP_ALLOW_CUSTOM_BASE:-}" == "1" ]] && return 0
...
cfp_api_reachable() {
  ...
  for m in "${candidates[@]}"; do
    for p in max_completion_tokens max_tokens; do
      ...
      # exit code already propagates without pipefail; pipefail (set above) additionally
      # catches a `printf` failure, which is real but not load-bearing for the
      # `|| http="000"` fallback below. Note: curl's own process environment
