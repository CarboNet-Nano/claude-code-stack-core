...
#
# ADR-087 D3b: set GKR_EVIDENCE_OUT to a file path (caller-created via
...
[...diff truncated at ${GKR_MAX_DIFF_BYTES} bytes for the review prompt...]"
  ...
  if [[ -n "${GKR_EVIDENCE_OUT:-}" ]]; then
    jq -nc \
      --arg vendor "xai" --arg family "xai" \
      --arg model_requested "$GKR_MODEL" \
      ...
      > "$GKR_EVIDENCE_OUT" 2>/dev/null || true
  fi
