...
#
# ADR-087 D3b: set DSR_EVIDENCE_OUT to a file path (caller-created via
# mktemp, truncated before the call) to receive one JSON evidence object on
# the SUCCESS path only. NOT an exported global. stdout contract unchanged.
#
...
[...diff truncated at ${DSR_MAX_DIFF_BYTES} bytes for the review prompt...]"
  ...
  if [[ -n "${DSR_EVIDENCE_OUT:-}" ]]; then
    jq -nc \
      --arg vendor "deepseek" --arg family "deepseek" \
      --arg model_requested "$DSR_MODEL" \
      ...
      > "$DSR_EVIDENCE_OUT" 2>/dev/null || true
  fi
