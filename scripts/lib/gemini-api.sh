...
#
# ADR-087 D3b: set GMN_EVIDENCE_OUT to a file path (caller-created via mktemp,
# truncated before the call) to receive one JSON evidence object on the
# SUCCESS path only (http_status, response_id, usage). NOT an exported
# global -- per-invocation sidecar, read once by the caller. stdout is
# UNCHANGED by this -- gmn_call's stdout contract stays frozen (only model
# text, ever). scripts/panel-review.sh is the only caller that sets it.
#
...
EOF
  ...
  # ADR-087 D3b: on the SUCCESS path only, write one JSON evidence object to
  # the caller-provided sidecar path (a file the caller created with mktemp
  # and truncated before the call). Never an exported global — the sidecar is
  # per-invocation and read exactly once by the caller. stdout contract is
  # frozen: this never changes what gmn_call prints (golden-fixture asserted,
  # ADR-087 test 100).
  if [[ -n "${GMN_EVIDENCE_OUT:-}" ]]; then
    jq -nc \
      --arg vendor "google" --arg family "gemini" \
      --arg model_requested "$GMN_MODEL" \
      --arg model_returned "$GMN_MODEL" \
      --argjson http_status "$http" \
      --arg response_id "$(printf '%s' "$resp" | jq -r '.responseId // empty' 2>/dev/null)" \
      --argjson input_tokens "$(printf '%s' "$resp" | jq -r '.usageMetadata.promptTokenCount // 0' 2>/dev/null)" \
      --argjson output_tokens "$(printf '%s' "$resp" | jq -r '.usageMetadata.candidatesTokenCount // 0' 2>/dev/null)" \
      '{vendor:$vendor, family:$family, model_requested:$model_requested, model_returned:$model_returned,
        http_status:$http_status, response_id:($response_id // null),
        usage:{input_tokens:$input_tokens, output_tokens:$output_tokens}}' \
      > "$GMN_EVIDENCE_OUT" 2>/dev/null || true
  fi
