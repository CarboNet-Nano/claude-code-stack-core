...
4a. **Emit a decision event (best-effort) via `pm decision`.** If this repo is
    a member of a PM portfolio (`config/portfolio.json`) and a
    `.claude/tracks/*.md` track applies to this work, journal the approved
    approach so the question-and-choice history is queryable later (REQ-147):
    ```
    node ~/.claude/tools/pm/bin.mjs decision --portfolio <portfolio> --track <track> \
      --question "<Deciding: line>" --choice "<recommended option>" \
      --option "<Option A>" --option "<Option B>" [--option "<Option C>"] \
      --rationale "<the 2-3 reasons>" --author skill
    ```
    Best-effort only: if `pm` tools are missing, or the portfolio/track can't
    be resolved, or the command errors, note it and move on — never brick
    brainstorming over this.
