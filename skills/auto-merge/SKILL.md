...
- **The MERGE itself must not be a GITHUB_TOKEN event either.** A merge enabled with the workflow's own token is performed as github-actions[bot], and GitHub's recursion guard then suppresses the push workflows on main — post-merge CI (including the Sweep's writer cadence) silently never runs, which B4 flags on every landing. The template therefore mints a token from the org's merge-bot GitHub App (org secrets MERGE_BOT_APP_ID / MERGE_BOT_PRIVATE_KEY, installed org-wide) and enables auto-merge as the app; app-caused merges trigger push workflows like a human's. A repo in an org without the app/secrets must create them first (one app per org, installed on all repositories).
