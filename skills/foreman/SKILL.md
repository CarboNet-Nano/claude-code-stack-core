...
6. **Apply domain modes (ADR-053 — path-scoped, multi-valued).** Source
   ...
   6f. Quick reference (authoritative source is `domain-modes.json`):
       - `financial-code`: validator + red-team + security-auditor on any
         merge; validator cross-checks real values. ADR-082 addition: after
         tester writes the test plan/fixtures for financial or write-path
         code, run the adversarial fixture review — an `oair_call` (OpenAI
         family, reviewer's transport; degrades to a labeled Claude pass
         if the key is unavailable, never blocks) asked to list the row
         states, inputs, and preconditions the fixture set CANNOT
         represent (void, zero, duplicate, already-fulfilled,
         already-dead). Check the response in at
         `docs/reviews/fixtures/<date>-<scope>.md`; tester answers each
         named gap (adds the case or records why not).
...
3b. **If dispatching `implementer`, declare the reviewed subject** (ADR-087
   G1). Include one line in the dispatch prompt naming the artifact the
   adversarial panel reviewed — the plan/ADR the implementation follows:

   ```
   Review-subject: docs/ADRs/<the-reviewed-artifact>.md
   ```

   The `review-gate` hook checks that line against a receipt minted from a
   genuine `scripts/panel-review.sh` run on that artifact (fresh, cross-family
   per the change class — see ADR-087 D3f/D4). **Currently ships in `warn`
   mode — it logs, it never blocks** (`guards.review_gate` flips to `on` only
   per ADR-087 D6's evidence bar). A missing line today produces a telemetry
   row, not a denial. Never paper over a deny — surface it.

   **The panel runner is invoked from the main thread — this is a rule
   (ADR-087 D3c).** A panel seat that mints no receipt does not count.
   Subagents (architecture-critic, red-team, reviewer, security-auditor,
   product-critic) may author the seat prompt and interpret the critique, but
   the call that produces evidence is the main thread's own
   `scripts/panel-review.sh` invocation (stdin = assembled context,
   `--subject <path>` for artifacts / `--diff <base>..<head>` for patches).

4. **Resolve the subagent's behavior-matrix cell** (REQ-122): run

   ```
   node ~/.claude/tools/pm/bin.mjs matrix resolve --agent <name> --domain <domain_mode> --sensitivity <sensitivity>
   ```

   (domain_mode/sensitivity from `stack-config.json`; omit either to fall back
   to `default`/`normal`). Hand the resolved **`{assertiveness, autonomy}`**
   cell to the subagent as dispatch context — e.g. append `Assertiveness:
   <value> — Autonomy: <value>` to the dispatch prompt. This is the same
   resolver PM reads for its own challenge level (Task 15): one resolver, two
   consumers. The command is read-only and never throws — an unknown agent,
   domain mode, or sensitivity still resolves to a real row from the shipped
   defaults and exits 0, so a bad or stale agent name never blocks dispatch.
   If the resolved response carries any `warnings` (an override lowering a
   shipped `gate` dial, REQ-125), surface them to the user before dispatching
   — same disclosure spirit as the domain-mode checkpoints in step 6 above.
   If the resolver **command itself** fails (missing `bin.mjs`, unreadable
   config, non-zero exit, or any other launch/parse error) — a broken
   environment, not a bad input value — do not stall dispatch: proceed with
   the sane default cell `{assertiveness: recommend, autonomy:
   decide-with-review}` and note the degradation in the dispatch context
   (e.g. `Assertiveness: recommend — Autonomy: decide-with-review
   (resolver unavailable, using default)`).
5. Invoke the subagent with a clear scope (link to architect-handoff.md, validator-report.md, etc.).
6. Wait for completion.
7. Read the subagent's report.
8. Move to next subagent (parallel where the team is parallel).
