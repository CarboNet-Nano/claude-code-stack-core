...
## What this skill does NOT catch (ADR-082)

A green /validate-output run says nothing about: disconnected code (built
but never wired), absent writes, read-path key mismatches, contract drift
between write conventions and read ceilings, over/under-broad read
predicates, or arithmetic that is only wrong on the production data
distribution. Those need the Sweep's wiring checks, the production-data
invariants runner (`tools/invariants`), and the scheduled human
walkthrough. The 29-bug audit behind ADR-082 found the write-landed gate
reaches 14% of real defects — treat this as one instrument, never the
gate.
