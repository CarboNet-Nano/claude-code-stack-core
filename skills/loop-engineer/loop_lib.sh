...
# called by both /carbonight (Step: model-fit receipt) and the Stop-hook fallback
...
# lesson compounds — /carbonight folds unresolved corrections into the next session.
...
loop_ultracode_active() {
  ...
  # The state file is keyed by session id, following the ADR-020 pattern that
  # design-approved.<sid>.json already uses. Without that key the file outlives
  # the session that wrote it: a single `/ultracode on` was observed still
  # active FIVE DAYS later, silently gating every session in between. A
  # "per-session opt-in" that cannot expire is not per-session.
  local _sdir="${LOOP_STATE_DIR:-${_loop_home}/.claude/session-state}"
  local _sid="${CLAUDE_CODE_SESSION_ID:-}"; _sid="${_sid//[^A-Za-z0-9._-]/_}"
  local f="$_sdir/ultracode-state.json"
  [[ -n "$_sid" && -f "$_sdir/ultracode-state.$_sid.json" ]] && f="$_sdir/ultracode-state.$_sid.json"

  if [[ -f "$f" ]]; then
    # An unkeyed legacy file is authoritative only when it names the session it
    # was written for and that session is this one. An unkeyed file with no
    # session_id belongs to some past session and is ignored — which lands on
    # ultracode OFF, the correct default, rather than inheriting a stale ON.
    local _owner _same=1
    if [[ "$f" == "$_sdir/ultracode-state.json" ]]; then
      _owner="$(jq -r '.session_id // ""' "$f" 2>/dev/null)"
      [[ -n "$_owner" && "$_owner" == "$_sid" ]] || _same=0
    fi
    if (( _same )); then
