...
description: The ONLY writer of Claude Code native settings.json. Sets exactly one allowlisted scalar leaf (model, outputStyle, statusLine preset, an enabledPlugins toggle, an mcpServers disabled flag, or the user-scope sandbox strictAllowlist toggle) via a hardened script — set-at-path, deny-by-default, atomic, cloud-gated. Invoked only by the /stack-config dashboard after a user confirmation; never edits hooks, env, permissions, or raw MCP command/args. Not for stack-config.json (that is /stack-config + the owning setting-skills).
...
| `/sandbox/network/strictAllowlist` | `true`/`false` — **user scope only** (`--scope user --confirm-global`; ADR-063 D5: the key is honored only in user/managed/CLI settings, so project scope is refused) |
