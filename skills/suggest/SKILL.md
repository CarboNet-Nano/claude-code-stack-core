...
```
• /carbonight (or: "close out the session") — capture session state before closing. [high]
```

**balanced** (default) — one line per result with a brief reason:
```
• /carbonight — close out the session
  Reason: you are ending a working session. Invoke: /carbonight or say "close out the session"
  Confidence: high
```
...
- confidence label
- if `origin` is `"live"`, append the suffix `(installed on this machine — not part of the stack)` to the result line (ADR-043)
