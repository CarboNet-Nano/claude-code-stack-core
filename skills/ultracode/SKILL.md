...
1. **Resolve state dir and session-keyed file.**
   ```bash
   ...
   [[ -n "$SID" ]] && FILE="$STATE_DIR/ultracode-state.$SID.json"
   ```
   The filename carries the session id, matching the ADR-020 pattern that
   `design-approved.<sid>.json` already uses. This is load-bearing, not
   cosmetic: an unkeyed file has no way to expire, and one was found still
   reading `active:true` **five days** after the session that wrote it, gating
   every session in between. Always stamp `session_id` in the body too, so a
   legacy unkeyed file can still prove which session owns it.
...
3. **on:**
   ```bash
   mkdir -p "$STATE_DIR"
   printf '{"active":true,"set_at":"%s","session_id":"%s"}\n' \
     "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$SID" > "$FILE"
   ```
...
4. **off:**
   ```bash
   mkdir -p "$STATE_DIR"
   printf '{"active":false,"session_id":"%s"}\n' "$SID" > "$FILE"
   ```
