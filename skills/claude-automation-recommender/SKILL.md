...
claude -p "fix lint errors in src/" --allowedTools Edit,Write --max-turns 20

# CI pipeline with structured output, capped so a runaway prompt can't spend
# past what the job is worth (print/-p mode only, both flags)
claude -p "<prompt>" --output-format stream-json --max-turns 20 --max-budget-usd 5 | your_command
```

Both are native CLI belt-and-suspenders caps, not a substitute for scoping
`--allowedTools` correctly. Size `--max-turns` to how many agentic turns the
job realistically needs; size `--max-budget-usd` from an actual cost
projection (see `/cost-gate`), not a guess.
