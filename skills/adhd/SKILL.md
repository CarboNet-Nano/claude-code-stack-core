...
tier_min: 2
upstream: https://github.com/UditAkhourii/adhd (skills/adhd)
vendored: 2026-08-14
---
...
**Step 3. Stack cost protection (added when this skill was vendored — not upstream).**

Ten Agent calls is a real cost, and this stack already has a place where
that decision belongs. Before Phase 1, state the expected spend in one line
and let the user stop you:

> This runs ~10 agent calls (roughly 5-10x a single answer). Proceed? (y / no)

Silence is not consent — no answer means abort. If the session has a hard
per-session cap set (`cost_protection.hard_cap_usd` in stack-config.json)
and this run would cross it, refuse and say so; do not ask the user to
approve past their own cap.
