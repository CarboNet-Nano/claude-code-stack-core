...
for repo in "${REPOS[@]}"; do
  ...
  (
    ...
    _title="Update $TENANT_ID standards to pack $PACK_VERSION"
    _body="Automated by the Claude Code Stack org reconciler (Job B, ADR-034 §5). Re-vendors the \`$TENANT_ID\` pack's \`standards/\` at pack version \`$PACK_VERSION\`."
    ...
    if command -v broker_push_and_pr >/dev/null 2>&1; then
      ...
      echo "lib/broker-push.sh did not load while the broker is live — refusing the direct path: $repo" >&2
      _brc=3
    fi
    if [ "$_brc" = "0" ]; then exit 0; fi
    if [ "$_brc" != "75" ]; then
      echo "broker refused push/PR for $repo (rc=$_brc)" >&2; exit 4
    fi
    echo "broker unavailable — direct git/gh fallback (pre-P5 path): $repo" >&2
