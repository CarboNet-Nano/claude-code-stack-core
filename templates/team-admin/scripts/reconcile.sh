...
#   STACK_REPO  public stack URL (default: github.com/CarboNet-Nano/claude-code-stack-core)

set -euo pipefail

CONFIG="${CONFIG:-config.yml}"
STACK_REPO="${STACK_REPO:-https://github.com/CarboNet-Nano/claude-code-stack-core}"
...
for repo in "${REPOS[@]}"; do
  ...
  if ( cd "$work"
    ...
    # ADR-075: seeding only ever writes when a directory is ABSENT, which is
    # what made every repo a permanently stale fork. Reconcile the ones that
    # already exist, using the clone we already have rather than a second
    # implementation. Refreshed files ride the same commit and PR below.
    if [ -f "$STACK_DIR/stack/lib/portable-core.sh" ]; then
      # shellcheck disable=SC1090
      CLAUDE_PLUGIN_ROOT="$STACK_DIR/stack" . "$STACK_DIR/stack/lib/portable-core.sh" 2>/dev/null || true
      if command -v pc_reconcile >/dev/null 2>&1 || declare -F pc_reconcile >/dev/null 2>&1; then
        CLAUDE_PLUGIN_ROOT="$STACK_DIR/stack" pc_reconcile "$PWD" 1 >/dev/null 2>&1 || true
      fi
    fi
    ...
    _title="Add Claude Code Stack cloud bootstrap"
    _body="Automated by the Claude Code Stack org reconciler. Adds \`.claude/hooks/cloud-bootstrap.sh\` + a SessionStart hook + portable-core skills so this repo's cloud sessions (web/iOS) self-install the stack. Stack version \`$VERSION\`. See https://github.com/bschonbrun/claude-code-stack/blob/main/docs/CLOUD.md"
    # D18 P4: GitHub writes go through the stack-broker where it is installed;
    # direct git/gh is the pre-P5 fallback (rc 75 = broker unavailable here).
    # shellcheck disable=SC1090
    [ -f "$HOME/.claude/lib/broker-push.sh" ] && . "$HOME/.claude/lib/broker-push.sh"
    _brc=75
    if command -v broker_push_and_pr >/dev/null 2>&1; then
      broker_push_and_pr "$PWD" "$ORG/$repo" "$BRANCH" "$default" "$_title" "$_body" >/dev/null
      _brc=$?
    elif command -v stack-broker >/dev/null 2>&1 && stack-broker pending --json >/dev/null 2>&1; then
      # Library missing but the broker is live: rc 75 ("no broker here") would
      # route a live door's judgement around itself. Refuse instead.
      log "lib/broker-push.sh did not load while the broker is live — refusing the direct path: $repo"
      _brc=3
    fi
    if [ "$_brc" = "75" ]; then
      log "broker unavailable — direct git/gh fallback (pre-P5 path): $repo"
      ...
      log "broker refused push/PR for $repo (rc=$_brc)"; exit 4
    fi
  ); then
