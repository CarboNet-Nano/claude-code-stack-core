...
const fixtureWithBudget = `---
track: budget-track
goal: Track with a spend estimate
updated: 2026-08-08
blocked_on:
prioritized: true
budget_usd: 10
...
test("REQ-111 amendment: parseTrack reads budget_usd as a Number", () => {
  const parsed = parseTrack(fixtureWithBudget);
  assert.equal(parsed.budget_usd, 10);
  assert.equal(typeof parsed.budget_usd, "number");
});

test("REQ-111 amendment: parseTrack omits budget_usd entirely when the frontmatter has none", () => {
  const parsed = parseTrack(fixture);
  assert.equal(parsed.budget_usd, undefined);
  assert.ok(!("budget_usd" in parsed), "budget_usd key must not appear at all when absent");
});

test("REQ-111 amendment: parseTrack round-trips budget_usd through updateTrack", () => {
  const updated = updateTrack(fixtureWithBudget, { state: "Still on track.", updated: "2026-08-09" });
  const reparsed = parseTrack(updated);
  assert.equal(reparsed.budget_usd, 10);
  assert.match(updated, /budget_usd: 10/);
});

test("REQ-111 amendment: updateTrack never fabricates budget_usd for a track that never had one", () => {
  const updated = updateTrack(fixture, { state: "Still on track.", updated: "2026-08-09" });
  assert.doesNotMatch(updated, /budget_usd/);
});
