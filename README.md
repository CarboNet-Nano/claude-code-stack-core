...
**Status:** Proprietary Lade IP (ADR-034/036). Being taken private; distributed to licensees under written agreement only. No support commitments.

## What this is

25 specialized subagents covering the full software development lifecycle, multi-provider model routing (Anthropic, OpenAI, Google, optional local Ollama), self-improving via monthly audits, and 5 installation tiers from "stop repeating mistakes" to "set it and forget it" enterprise.

The full roster of 24 subagents installs at **every tier**, from Tier 0 up — tiers gate skills, hooks, and infrastructure, not which agents you have. The 25th, `local-ops`, stays at Tier 5 because it requires a local Ollama install. **Tier 4 is the default** for new projects.
...
Proprietary — all rights reserved. See `LICENSE`. Versions published on or before 2026-07-26 were Apache 2.0 (rationale in `docs/ADRs/001-license-apache-2.md`); that grant is irrevocable for copies obtained while it was in effect. This and later versions require a written license from Lade.
